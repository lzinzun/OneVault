...
class OpenSecretDialogFragment:DialogFragment() {
    ...
    private lateinit var secret_tv:TextView
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        var rootView:View = inflater.inflate(R.layout.open_secret_dialog_fragment, container, false)
        var doneButton = rootView.findViewById<Button>(R.id.done_btn)
        var acctUserId:String = arguments?.get("ACCT_ID").toString()
        secret_tv = rootView.findViewById(R.id.revealed_secret_tv)

        val revealedData = getEncryptedData(acctUserId)

        secret_tv.setText(revealedData)
    ...
    private fun getEncryptedData(acctUserId:String):String{
        ...
        var decryptedData = aesEncryptionUtil.decrypt("662ede816988e58fb6d057d9d85605e0",encryptedData)
